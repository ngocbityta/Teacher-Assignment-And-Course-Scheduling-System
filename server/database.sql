...
CREATE TABLE IF NOT EXISTS CoursePreference (
                                                id VARCHAR PRIMARY KEY,
                                                semester_id VARCHAR NOT NULL REFERENCES Semester(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS Schedule (
                                        ...
                                        semester_id VARCHAR NOT NULL REFERENCES Semester(id) ON DELETE CASCADE,
    teacher_id VARCHAR NOT NULL REFERENCES Teacher(id) ON DELETE CASCADE,
    section_id VARCHAR NOT NULL REFERENCES Section(id) ON DELETE CASCADE,
    classroom_id VARCHAR NOT NULL REFERENCES Classroom(id) ON DELETE CASCADE,
    day INT NOT NULL CHECK (day >= 1 AND day <= 7),
    period period_enum NOT NULL
    );
